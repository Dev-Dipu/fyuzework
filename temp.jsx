...
// Replace the filter panel section with this updated code:
{/* filter panel */}
<div
    className={`absolute w-full pb-12 bottom-10 rounded-t-4xl flex justify-end px-6 transition-all duration-500 ease-in-out font-[inter] font-medium ${
        isDark ? "bg-[#060606]" : "bg-white"
    } ${isFilterOpen ? "h-[80vh] py-6 overflow-y-auto" : "h-auto py-2.5"}`}
>
    {/* filter */}
    <div className={`w-full transition-all duration-500 ${isFilterOpen ? "h-auto opacity-100" : "h-0 opacity-0 overflow-hidden"}`}>
        {/* Header */}
    ...
    {/* Toggle Button - Only show when filter is closed */}
    {!isFilterOpen && (
        <div 
            onClick={() => setIsFilterOpen(true)}
            className="flex h-fit items-center gap-1 cursor-pointer hover:text-white transition"
        >
            <Image
                height={1}
                width={20}
